...
<body class="bg-slate-100 text-slate-800">
    <div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-200 p-6 lg:p-10">
        <div id="app" class="mx-auto max-w-6xl rounded-2xl border border-slate-200 bg-white p-6 shadow-lg">
            <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                <div>
                    <p class="text-sm font-semibold uppercase tracking-[0.2em] text-sky-600">Foogu CMS</p>
                    <h1 class="mt-1 text-3xl font-semibold text-slate-900">Backend API Endpoint & Fitur</h1>
                    <p class="mt-2 max-w-3xl text-sm text-slate-600">
                        Halaman ini menampilkan daftar endpoint backend yang tersedia, fitur masing-masing, dan status
                        aksesnya.
                    </p>
                </div>
                <div
                    class="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-sm font-medium text-emerald-700">
                    Status backend: <span id="posts-status">Sedang memuat...</span>
                </div>
            </div>

            <div class="mt-8 grid gap-4 lg:grid-cols-2">
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                    <h2 class="text-lg font-semibold text-slate-900">Autentikasi</h2>
                    <ul class="mt-3 space-y-2 text-sm text-slate-600">
                        <li><strong>POST /api/register</strong> — registrasi user baru</li>
                        <li><strong>POST /api/login</strong> — login dan dapatkan token</li>
                        <li><strong>POST /api/logout</strong> — logout token aktif</li>
                        <li><strong>GET /api/me</strong> — melihat data user login</li>
                    </ul>
                </div>

                <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                    <h2 class="text-lg font-semibold text-slate-900">Post Management</h2>
                    <ul class="mt-3 space-y-2 text-sm text-slate-600">
                        <li><strong>GET /api/posts</strong> — lihat daftar post</li>
                        <li><strong>GET /api/posts/{slug}</strong> — lihat detail post</li>
                        <li><strong>POST /api/posts</strong> — buat post baru</li>
                        <li><strong>PUT/PATCH /api/posts/{slug}</strong> — edit post</li>
                        <li><strong>DELETE /api/posts/{slug}</strong> — hapus post</li>
                    </ul>
                </div>
            </div>

            <div class="mt-6 rounded-xl border border-slate-200 bg-white p-4">
                <h2 class="text-lg font-semibold text-slate-900">Data yang saat ini tersedia dari backend</h2>
                <div id="posts-list" class="mt-4 grid gap-4"></div>
            </div>
        </div>
    </div>
